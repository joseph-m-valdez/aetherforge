  ...
  return (
    <aside className={cn('flex h-full flex-col gap-3.5 p-3.5', className)}>
      <div className="flex items-baseline justify-between">
        <h2 className="text-base font-semibold uppercase tracking-wide text-text-bright">Fleet</h2>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-sm font-bold uppercase tracking-hud" style={{ color: fleet.color }}>
          {fleet.name}
        </span>
...
function VehicleRow({
  ...
  const battery = vehicle.battery ?? -1

  function fmt(v: number | undefined, render: (n: number) => string): string {
    if (!v) return '-'
    return render(v)
  }
      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        <div className="flex items-center gap-2 pr-1 pl-2">
          ...
          >
            ...
            <span className={cn('ml-auto font-mono text-sm font-semibold', batteryClass(battery))}>
              {battery < 0 ? '—' : `${vehicle.battery}%`}
            </span>
          </button>
          ...
          <CollapsibleTrigger asChild>
            <Button
              ...
              size="icon-sm"
              aria-label={
                open ? `Hide ${vehicle.callsign} telemetry` : `Show ${vehicle.callsign} telemetry`
              }
            >
              {open ? (
                <ChevronDown className="size-4" aria-hidden />
        ...
        <CollapsibleContent asChild>
          <dl className="m-0 grid grid-cols-3 gap-x-2.5 gap-y-1.5 bg-panel-2 px-3 pt-2 pb-3 pl-6">
            <Field
              k="GS"
              v={fmt(t.groundSpeed, (groundSpeed) => `${groundSpeed.toFixed(1)} m/s`)}
            />
            <Field k="AS" v={fmt(t.airSpeed, (airSpeed) => `${airSpeed.toFixed(1)} m/s`)} />
            <Field
              k="VS"
              v={fmt(t.verticalSpeed, (verticalSpeed) => `${signed(verticalSpeed)} m/s`)}
            />
            <Field k="ALT" v={fmt(t.altitudeRel, (altitudeRel) => `${altitudeRel.toFixed(1)} m`)} />
            <Field k="HDG" v={fmt(t.heading, (heading) => `${Math.round(heading)}°`)} />
          </dl>
        </CollapsibleContent>
      </Collapsible>
    </li>
